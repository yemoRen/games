...
import { MAP_BRANCH_COUNT, EXTRACT_POINT_COUNT, ZONE_POOL_SIZE } from './types';
...
/** v1.0.11：生成一张大地图的专属「区域图 16 区」（贴合主题）。
 *  id 前缀 = mapId-sub-；与 branches() 的 10 区不冲突（不同命名空间）。 */
function subZones(mapId: string, defs: Array<[name: string, flavor: string]>): ZoneBranch[] {
  if (defs.length !== ZONE_POOL_SIZE) {
    throw new Error(`地图 ${mapId} 区域图 16 区数量必须为 ${ZONE_POOL_SIZE}，实际 ${defs.length}`);
  }
  return defs.map(([name, flavor], i) => ({ id: `${mapId}-sub-${i}`, name, flavor }));
}
...
/** v1.0.11：敌人数值随区域危险度（危N）放大，恢复「越深越险」的风险/回报梯度。
 * 取值 = 敌人基础六维的乘数（深度词缀叠加后的整体再放大），由 buildEnemyUnit 在战斗构建时套用。 */
// v1.0.11：敌人数值随区域危险度（危N）放大，恢复「越深越险」梯度。
// 注意：战斗引擎存在「数值悬崖」——敌人属性提升 ~5% 即可能从「惨胜」翻转为「秒杀 0 胜率」。
// 故此处统一保持 1.0（不盲目放大），难度主要通过「等级门槛」(DANGER_LEVEL_REQ) 控制：
// 低等级无法进入高危区，从而把「副本难度」交还给等级与装备成长。如需整体提难，改此表并充分实测。
// v1.1.7：恢复「危险度越高敌人越强」的梯度（此前全 1.0 导致后期割草无风险）。
// 每级约 +10% 基础六维，封顶 +60%；经测试对中后期仍保持可观掉血，同时避免 1 级新手被碾压。
export const ENEMY_DANGER_SCALE: Record<number, number> = {
  1: 1.0,
  2: 1.10,
  3: 1.20,
  4: 1.30,
...
export const DANGER_ZONES: DangerZone[] = [
  {
    ...
    subZones: subZones('apartment', [
      ['公寓外围', '锈蚀的旋转门早已卡死，门廊堆满无主邮包。'],
      ['一楼大堂', '信箱倾倒，泛黄的邮件散落一地，叫号屏黑着。'],
      ['消防楼梯井', '每走一步都在吱呀，回声里像有人跟着你爬。'],
      ['二层住户', '户户虚掩的房门，墙上还挂着发黄的全家福。'],
      ['阁楼储物间', '隔热棉剥落，旧物堆里偶见被遗忘的相册。'],
      ['外挂铁梯', '悬在三楼外墙的逃生梯，锈得只能容一人通过。'],
      ['天台水箱', '水箱早已干涸，从这里能望见半座死城。'],
      ['楼底锅炉房', '煤灰与锈水味交织，墙上的仪表永远停在灾难当天。'],
      ['地下储藏室', '杂物堆积如山，霉味刺鼻，翻找费时但常有遗漏的物资。'],
      ['火灾楼层', '焦黑的门框仍在滴水——不是水，是冷凝。'],
      ['邻居遗物室', '几箱未拆封的搬迁纸箱，贴着再也无人认领的姓名。'],
      ['配电间废墟', '跳闸的火花早已熄灭，铜芯却被人撬走了大半。'],
      ['停车场夹层', '报废车辆挤作一团，偶尔有东西在车底窸窣。'],
      ['顶层复式', '家具尽数掀翻——这里似乎被什么东西长期占据。'],
      ['楼道深处的门', '贴着"私人勿入"的纸片，门后有什么在等。'],
      ['楼王巢穴', '腐臭扑面。这层楼的"房东"，还没有搬走。'],
    ]),
  },
  {
    ...
    subZones: subZones('hospital', [
      ['门诊大厅', '候诊椅上坐满了"永不叫号"的病人，叫号屏黑着。'],
      ['挂号走廊', '叫号屏还亮着一半，走廊两侧诊室门大开。'],
      ['药房柜台', '铁柜大多被撬开，但总有漏网的好东西。'],
      ['一层急诊', '担架横陈，抢救室的灯还闪着将灭的绿光。'],
      ['X光影像室', '观片灯还夹着几张未取走的胸片。'],
      ['CT扫描区', '环形探测器仍在低鸣，床架被人推离了原位。'],
      ['住院部一层', '病床排成长龙，帘子后面有动静。'],
      ['手术室区', '无影灯早已熄灭，器械盘里的东西却不是器械。'],
      ['护士站值班室', '白板上"当班"二字仍清晰，墨迹早已干透。'],
      ['住院部三层', '护理站的白板写着某床"情况稳定"——三天前。'],
      ['病历档案室', '整柜整柜的纸病历，有些姓氏被人匆匆划去。'],
      ['血库', '冷藏柜断电已久，血袋在暗处泛着不祥的色泽。'],
      ['停尸房', '冷柜抽屉拉出一半，里面空得让人发毛。'],
      ['太平间深处', '墙上的编号被人用黑漆涂掉又重写，反反复复。'],
      ['院长办公室', '保险柜、遗书与一杯长霉的咖啡。'],
      ['尸潮之心', '整层楼的心跳声同频震动——尸潮之主就在此。'],
    ]),
  },
  {
    ...
    subZones: subZones('outskirts', [
      ['公路收费站', '横杆早已砸断，收费亭成了狙击位。'],
      ['抛锚车队', '逃难车队长龙，后备箱里常有遗落的行李。'],
      ['路旁野店', '招牌只剩"招"字半边，玻璃柜台下藏着子弹。'],
      ['加油站废墟', '油罐区警戒线还挂着，火星就是灾难。'],
      ['汽车旅馆', '房间按小时计费——现在按命计费。'],
      ['废弃农田', '枯死的玉米地里藏着灌溉渠与地窖。'],
      ['牲畜圈栏', '栅栏倾倒，围场里的蹄印通向密林深处。'],
      ['流浪者营地', '篝火余烬未冷，住户去向不明。'],
      ['农舍残垣', '屋顶塌了一半，烟囱却像被人重新砌过。'],
      ['灌溉渠泵房', '泵机的铁壳被人撬开，里面不是泵，是弹药箱。'],
      ['修车厂', '千斤顶与残骸之间，拾荒客最爱的淘金地。'],
      ['收割机坟场', '锈成骨架的农机排成行，地里的管道仍在滴油。'],
      ['仓储棚户', '成排的货棚，铁皮墙后别有洞天。'],
      ['瞭望木屋', '木板拼接的塔楼，登顶需一根一根踩稳。'],
      ['哨戒塔楼', '废墟帮的瞭望塔——登顶即暴露，风景绝佳。'],
      ['拾荒王大帐', '铁钩与猎物挂满帐帘，王座由废车壳铸成。'],
    ]),
  },
  {
    ...
    subZones: subZones('military', [
      ['外围铁丝网', '锈丝网上挂着旧警报器，钻过时千万别碰响。'],
      ['探照灯塔', '灯罩早被砸碎，电线却被重新接好，还在扫视。'],
      ['哨卡岗亭', '沙袋工事后的岗亭里，执勤名单还压在桌上。'],
      ['装甲残骸带', '瘫痪的装甲车队列，炮塔仍指向来路。'],
      ['弹药堆场', '野战弹药箱堆叠如墙——撬开就是硬通货。'],
      ['油料囤积站', '油桶锈迹斑斑，泄漏的油渍引来几只老鼠。'],
      ['车库维修间', '液压平台悬着一辆修到一半的运兵车。'],
      ['直升机停机坪', '跑道裂缝里长出杂草，标志"H"被涂成笑脸。'],
      ['通讯机房', '无线电台沙沙作响，像是有人在循环呼叫。'],
      ['雷场边沿', '木桩拉起的警戒线内，泥土仍会轻轻颤动。'],
      ['军官宿舍', '私人储物柜上着锁，里面有比军饷更好的东西。'],
      ['废弃营房', '床铺整齐，被褥却已霉成一片。'],
      ['战俘营栏', '铁丝网内散落着布条与弹壳，铁门锈死半开。'],
      ['地下军械库', '防爆门半开，枪架上的军用装备近在眼前。'],
      ['指挥掩体', '沙盘上插着末日当天的兵力标记。'],
      ['督战官阵地', '机枪碉堡转动——督战官·灰烬仍在执行最后一条军令。'],
    ]),
  },
  {
    ...
    subZones: subZones('subway', [
      ['入站口大厅', '扶梯停在半途，闸机上的灰尘被人拨开过。'],
      ['售票亭', '玻璃被砸碎，换乘图还挂在墙上，却再没用了。'],
      ['闸机口', '半开的铁闸发出吱呀声，像在清点每一个进来的人。'],
      ['候车月台', '座椅下塞着无人认领的行李，灯管只剩一根还亮。'],
      ['电力牵引站', '高压柜仍带电，绝缘手套被人叠好放在一边。'],
      ['轨道维修道', '扳手与信号灯还摆在原位，工具上的指纹还在。'],
      ['侧线岔口', '岔道深处有微光，也可能是反光。'],
      ['维修工区', '工具墙整齐如初，值班表停在大停电那天。'],
      ['积水区段', '齐腰深的死水里，你的每一步都是钟声。'],
      ['排水泵站', '锈蚀的泵机还在抽水，每一声都像心跳。'],
      ['通风管道', '管道内壁残留的油渍早已发黑，空气带着腥味。'],
      ['站台二号线', '积水没过脚踝，水面涟漪从黑暗中扩散而来。'],
      ['调度室', '调度台上压着一张再也不会发出的车次表。'],
      ['应急出口', '防火门上挂着疏散图，箭头指向"地面"。'],
      ['隧道深处', '手机信号全无，洞壁上开始出现黏稠的分泌物。'],
      ['巢母洞窟', '虫卵铺满洞壁，每一次心跳都来自巢母本身。'],
    ]),
  },
  {
    ...
    subZones: subZones('research', [
      ['地面废墟', '研究所的地上部分早已烧穿，电梯井是唯一入口。'],
      ['电梯井', '检修梯狭长幽深，每层门缝都透着不同的光。'],
      ['保安岗哨', '警卫室弹药充足——他们当年显然需要。'],
      ['监控中心', '上百块屏幕只亮着一半，回放停在末日当天 04:12。'],
      ['实验层B1', '培养皿碎裂满地，标签写满编号与警告。'],
      ['细胞培养室', '培养舱里悬浮的东西，形状正在变化。'],
      ['动物实验区', '笼门从里面被撞开的痕迹遍布。'],
      ['标本陈列室', '玻璃柜里的编号被人用马克笔反复涂改。'],
      ['数据机房', '服务器仍在低鸣，跑着一份永远完不成的分析。'],
      ['样本冷库', '液氮罐还在结霜，封条被撕开过又重新贴上。'],
      ['生化实验室', '气密门内侧的抓痕深达钢板。'],
      ['武器试验场', '靶纸钉满一面，弹孔分布出奇地整齐。'],
      ['档案密室', '密码锁被人暴力破开，纸页散了一地。'],
      ['深层禁地', '警示灯全红——"严禁入内"在这里是最低级的警告。'],
      ['自毁控制台', '红色按钮的护盖被人打开又合上，反复多次。'],
      ['巨兽培养舱', '零号实验体睁开了眼睛。培养舱是它自己打开的。'],
    ]),
  },
  {
    ...
    subZones: subZones('nuclear', [
      ['辐射缓冲带', '辐射读数在盖革计数器上碎成连续的长音。'],
      ['死城街道', '橱窗里的模特还保持着逃跑的姿势。'],
      ['融毁车辆带', '玻璃化车壳黏在路面上，像蜡泪。'],
      ['沾染通道', '墙面覆着厚厚的铅漆，是有人想掩盖什么。'],
      ['通风竖井', '井壁滚烫，热风把辐射尘吹成金色的雾。'],
      ['地下管廊', '管线网络如迷宫，图纸在这里比枪有用。'],
      ['应急控制室', '仪表盘蒙尘，唯独"反应堆状态"的红灯还在跳。'],
      ['冷却塔基座', '巨塔的阴影里，温度反而更低——也更安静。'],
      ['反应堆外围', '铅制屏障倾倒在地，谁挪动了它们？'],
      ['燃料池废墟', '池水早已干涸，底部残留的蓝色晶体仍在发光。'],
      ['控制棒舱室', '仪表盘全部爆表，唯独一台还在走时。'],
      ['防爆门厅', '三道防爆门只关了一道，剩余两道的锁都已熔毁。'],
      ['通讯铁塔残骸', '天线折成直角，地面散落未发出的求救电报。'],
      ['排水阀室', '锈死的阀门被人用撬棍硬生生拧开半圈。'],
      ['熔毁核心区', '岩浆般的核心余烬仍在呼吸。'],
      ['领主王座', '战争领主端坐于核心之上——废土尽头，它即秩序。'],
    ]),
  },
...
export function generateZoneGraph(theme: DangerZone, rng: () => number): ZoneGraph {
  // v1.0.11：优先使用地图专属 16 区池（贴合主题），fallback 到全局 ZONE_POOL
  const pool = theme.subZones ?? ZONE_POOL;
